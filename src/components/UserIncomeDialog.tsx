...
import { UserIncomeForm } from "@/components/UserIncomeForm";
...
export function UserIncomeDialog({
	...
	const totalMonthly = users.reduce(
		(sum, user) =>
			sum + calculateMonthlyIncome(user.incomeAmount, user.payFrequency),
		0,
	);
		...
		<Dialog open={open} onOpenChange={setOpen}>
			...
			<DialogContent className="sm:max-w-[600px]">
				...
				<div className="space-y-4 py-4">
										...
										<div className="flex items-start justify-between gap-4">
											<div className="flex-1 min-w-0">
												...
												<div className="text-sm text-muted-foreground mt-1">
													{formatCurrency(user.incomeAmount)}{" "}
													{getPayFrequencyLabel(user.payFrequency)}
												</div>
					...
					{/* Add Button */}
					<UserIncomeForm onAddUser={onAddUser} />
				</div>
			</DialogContent>
		</Dialog>
	);
